...
def get_stats():
    ...
    stats = {}
    cursor.execute("SELECT AVG(eval_score) FROM eval_records")
...
def get_model_summary_stats():
    ...
    conn = get_connection()
    query = """
        SELECT model_name, AVG(eval_score) as avg_score, COUNT(*) as test_count
        FROM eval_records
        GROUP BY model_name
...
def get_model_detail_stats(model_name):
    ...
    conn = get_connection()
    query = """
        SELECT c.title as case_title, AVG(r.eval_score) as avg_score, COUNT(*) as run_count
        FROM eval_records r
        JOIN test_cases c ON r.case_id = c.id
        WHERE r.model_name = ?
        GROUP BY r.case_id
...
def get_case_summary_stats():
    ...
    conn = get_connection()
    query = """
        SELECT c.id as case_id, c.title as case_title, AVG(r.eval_score) as avg_score, COUNT(*) as total_runs
        FROM eval_records r
        JOIN test_cases c ON r.case_id = c.id
        GROUP BY c.id
        ORDER BY avg_score DESC
...
def get_case_model_ranking(case_id):
    ...
    conn = get_connection()
    query = """
        SELECT model_name, AVG(eval_score) as avg_score, COUNT(*) as run_count
        FROM eval_records
        WHERE case_id = ?
        GROUP BY model_name
        ORDER BY avg_score DESC
